{ ... }:

{
  services.dunst = {
    enable = true;
    settings = {
      global = {
        monitor = 0;
        follow = "mouse";
        enable_posix_regex = true;
        width = 300;
        origin = "top-right";
        offset = "(20,20)";
        scale = 0;
        notification_limit = 20;
        progress_bar = true;
        progress_bar_height = 10;
        progress_bar_frame_width = 0;
        progress_bar_min_width = 125;
        progress_bar_max_width = 250;
        progress_bar_corner_radius = 4;
        indicate_hidden = "yes";
        transparency = 10;
        separator_height = 2;
        padding = 8;
        horizontal_padding = 8;
        text_icon_padding = 10;
        frame_width = 5;
        gap_size = 5;
        separator_color = "auto";
        sort = "yes";
        line_height = 3;
        markup = "full";
        format = "%s\n%b";
        alignment = "left";
        vertical_alignment = "center";
        show_age_threshold = 60;
        ellipsize = "middle";
        ignore_newline = "no";
        stack_duplicates = true;
        hide_duplicate_count = false;
        show_indicators = "yes";
        enable_recursive_icon_lookup = true;
        icon_position = "left";
        min_icon_size = 64;
        max_icon_size = 128;
        sticky_history = "yes";
        history_length = 20;
        browser = "/usr/bin/xdg-open";
        always_run_script = true;
        title = "Dunst";
        class = "Dunst";
        ignore_dbusclose = false;
        force_xwayland = false;
        force_xinerama = false;
        mouse_left_click = [ "context" "close_current" ];
        mouse_middle_click = [ "do_action" "close_current" ];
        mouse_right_click = "close_all";
        corner_radius = 10;
        icon_corner_radius = 10;
        font = "Mononoki Nerd Font Mono 10";
        dmenu = "/usr/bin/wofi --dmenu --prompt dunst:";
        icon_theme = "Tela-circle-dracula,Wallbash-Icon,Tela-circle-grey,Tela-circle-pink,Tela-circle-purple,Tela-circle-black,Tela-circle-green,Tela-circle-yellow,Tela-circle-blue,Gruvbox-Plus-Dark,Nordzy";
      };

      urgency_low = {
        background = "#6592A380";
        foreground = "#CCF1FFE6";
        frame_color = "#6592A303";
        icon = "./icons/hyprdots.svg";
        timeout = 5;
      };

      urgency_normal = {
        background = "#3A4A6B80";
        foreground = "#CCDDFFE6";
        frame_color = "#3A4A6B03";
        icon = "./icons/hyprdots.svg";
        timeout = 5;
      };

      urgency_critical = {
        background = "#f5e0dc";
        foreground = "#1e1e2e";
        frame_color = "#f38ba8";
        icon = "./icons/critical.svg";
        timeout = 0;
      };

      experimental = {
        per_monitor_dpi = true;
      };

      telegram = {
        desktop_entry = "^Telegram Desktop$";
        new_icon = "telegram-desktop";
      };

      "Type-1" = {
        appname = "HyDE Alert";
        format = "<b>%s</b>";
      };

      "Type-2" = {
        appname = "Hypr Notify";
        format = "<span size=\"250%%\">%s</span>\n%b";
      };
    };
  };
}
